...
INSERT INTO `actividad_proyecto` (`actividad_proyecto_id`,`ap_nombre`,`ap_fase`) VALUES 
 (1,'DETERMINAR LAS ESPECIFICACIONES',1),
 (2,'ESPECIFICAR EL MODELO',1),
 (3,'ESTABLECER METODOLOGÍA DE',1),
 (4,'ESTRUCTURAR LA PROPUESTA',1),
...
INSERT INTO `competencias` (`competencias_id`,`competencia_codigo`,`competencia_nombre`,`competencia_perfil`) VALUES 
 (1,'220501092','Establecer requisitos de la solución de software de acuerdo con estándares y procedimiento técnico','Tecnólogo o profesional en sistemas y afines, con conocimientos en Ingeniería de requisitos, elaboración y aplicación de instrumentos para recolección de información, trabajo por procesos, estadística descriptiva y elaboración de informes. '),
...
INSERT INTO `fichas` (`fichas_id`,`ficha_nombre`,`ficha_inicio`,`ficha_finalizacion`,`ficha_programa`) VALUES 
 (1,'2397050','06/02/2024','06/04/2024',1),
 (2,'2397051','08/02/2024','06/02/2026',3);
...
CREATE TABLE `horario` (
  ...
  `horario_instructor` int NOT NULL,
  ...
  `horario_ficha` int DEFAULT NULL,
...
INSERT INTO `instructores` (`instructores_id`,`instructor_numid`,`instructor_nombres`,`instructor_profesion`,`instructor_especialidad`,`instructor_perfil`,`instructor_correo`) VALUES 
 (1,'12435718','José David Montesino Hoyos',1,3,'Programación, implantación','jdmontesino@sena.edu.co'),
...
INSERT INTO `planeacion` (`planeacion_id`,`planeacion_programa`,`planeacion_ap`,`planeacion_rap`,`planeacion_aa`) VALUES 
 (1,1,1,1,'Teoria general de los sistemas');
...
INSERT INTO `postgrado` (`postgrado_id`,`postgrado_nombre`,`postgrado_titulo`) VALUES 
 ...
 (3,'TRANSFORMACION DIGITAL','MAESTRIA');
...
INSERT INTO `programa` (`programa_id`,`programa_codigo`,`programa_nombre`) VALUES 
 (1,'1','Análisis y desarrollo de software'),
 (3,'202020','GAnaderia'),
...
DROP TABLE IF EXISTS `vista_programacion`;
DROP VIEW IF EXISTS `vista_programacion`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_programacion` AS select `programacion`.`programacion_id` AS `vista_programacion_id`,`vista_planeacion`.`programa_nombre` AS `programa_nombre`,`fichas`.`ficha_nombre` AS `ficha_nombre`,`vista_planeacion`.`fase` AS `fase`,`vista_planeacion`.`ap_nombre` AS `ap_nombre`,`vista_planeacion`.`competencia_nombre` AS `competencia_nombre`,`vista_planeacion`.`rap_nombre` AS `rap_nombre`,`vista_planeacion`.`planeacion_aa` AS `planeacion_aa`,`instructores`.`instructor_nombres` AS `instructor_nombres`,`programacion`.`prog_estado` AS `prog_estado` from (((`vista_planeacion` join `programacion`) join `instructores`) join `fichas`) where ((`programacion`.`prog_planeacion` = `vista_planeacion`.`vista_planeacion_id`) and (`programacion`.`prog_ficha` = `fichas`.`fichas_id`) and (`programacion`.`prog_instructor` = `instructores`.`instructores_id`));
