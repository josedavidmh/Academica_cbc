...
function consulta_general(variable){
    ...
    let url = host+"consulta_general/"+variable;
              fetch(url)
              ...
              const visualizar = (data) => {
                  ...
                  for (var i = 0; i < data.datos.length; i++) { 
                     ...
                     b+=`<td><button type='button' class="btn btn-info" onclick="location.href = 'edit.html?variable1=${variable}&variable2=${data.datos[i][0]}'"> <img src='img/edit.png' height ='20' width='20'/></button></td>
                     <td><button type='button' class="btn btn-warning" onclick="eliminar('${variable}',${data.datos[i][0]})"> <img src='img/delete.png' height ='20' width='20'/></button></td></tr>`    
                    }
                    b2="";
...
function eliminar(table,codigo){
  ...
  .then((borrar) => {
    if (borrar) {
      ...
      let url = host+"eliminar/"+tabla_buscar+"/"+codigo;
      fetch(url, {
        method: 'DELETE',
      })
      .then( response => response.json() )
...
function registrar(table){
  tabla_buscar=columnas[table][1]
  let url = host+"registro/"+tabla_buscar;
  var data= {};
...
let url = host+"consulta_individual/"+tabla+"/"+codigo;
fetch(url)
  .then( response => response.json())
  .then( data => visualizar(data) )
  .catch( error => console.log(error) )
...
function modificar(table,id){
  tabla_buscar=columnas[table][1]
  let url = host+"actualizar/"+tabla_buscar+"/"+id;
var data= {};
  for (var k=0;k<columnas[table][0].length;k++)
                          {
                          j=k+1
...
function combo(tabla,elemento,clase,valor,valor2,valor3,seleccion,col){
  ...
  let url = host+"consulta_general/"+tabla;
            fetch(url)
                .then( response => response.json())
                .then( data => visualizar(data) )
                .catch( error => console.log(error) )
  ...
  document.getElementById('titulo2').innerHTML="Registrar "+datosjson[tabla][0]
    ...
    if (l<6 && l%2!=0)
      ...
      function consulta_avance(){
        ...
        let url = host+"programacion_resumen/"+ficha;
                  fetch(url)
                      .then( response => response.json())
                      .then( data => visualizar(data) )
                      .catch( error => console.log(error) )
