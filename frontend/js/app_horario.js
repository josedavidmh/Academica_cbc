...
function agregar_opcion(tabla_buscada,id_select,valor,valor2,indice,campo){
    ...
    if (campo!='NA'){
         url = host+"programacion_ficha/"+campo;
         limpia_combo(combo);
    }
    else{
        url = host+"consulta_general/"+tabla_buscada;  
    }
                  fetch(url)
...
function consulta_horario(vficha,vstart,vend){
    ...
    let modificar=0;
    let url = host+"horario_ficha/"+"horario/"+vficha;
                 fetch(url)
                  .then( response => response.json())
                  .then( data =>  visualizar(data) )
                  .catch( error => console.log(error))
...
function registro_horario(data,id){
    ...
    if (c==0){
        url = host+"registro/horario";
        metodo="POST";
        msgerror="Error en registro:"
        msgsuccess="Registro agregado:"
    }
    else{
        url = host+"actualizar/horario/"+id;
        metodo="put";
